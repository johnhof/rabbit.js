...
PubSocket.prototype.publish = function(topic, chunk, encoding, dest) {
  var ch = this.ch;

  if (!dest) {
    return this.publishAll(topic, chunk, encoding);
  } else{

    // if the destination is not belong to the socket, error out
    if (this.pubs.indexOf(dest) == -1) {
      console.error("Destination " + dest + " doesn't belong to the socket");
      return false;
    }
...
PubSocket.prototype.publishAll = function(topic, chunk, encoding) {
  var ch = this.ch;
...
function SubSocket(channel, opts) {
  ...
  var setup = channel.then(function(ch) { return ch; });
...
SubSocket.prototype.connect = function(source, topic, callback) {
  ...
  var ch = this.ch, queue = "results." + topic, self = this;
  ch.assertQueue(queue, {durable: false, autoDelete: false, noAck:true})
  if (this.options.noCreate) {
    ...
    ch.assertExchange(source,
      ...
      .then(function(ok) {
        ...
        //return ch.consume(queue, callback, {noAck:true, exclusive:false})
      })
      .then(callback);
  }
};
